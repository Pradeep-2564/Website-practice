...
const pages = ['Home', 'Support', 'Blog', 'Account'];
const settings = ['Profile', 'Account', 'Dashboard', 'Logout'];

function NavBar() {
  const [anchorElNav, setAnchorElNav] = React.useState(null);
  const [anchorElUser, setAnchorElUser] = React.useState(null);
  const [currentPage, setCurrentPage] = React.useState('Home'); // Track the current page
        ...
        <Container maxWidth="xl">
          <Toolbar disableGutters>
            ...
            <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
              {pages.map((page) => (
                <Button key={page} sx={{ my: 2, color: 'white', display: 'block' }} onClick={() => handlePageChange(page)}>
                  {page}
                </Button>
              ))}
            </Box>

            <Search sx={{ mr: 20 }} style={{ width: '500px' }}>
              <SearchIconWrapper>
                <SearchIcon />
              </SearchIconWrapper>
